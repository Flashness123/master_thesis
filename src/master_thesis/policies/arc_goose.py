...
class ActionModel(nn.Module):
  ...
  def __init__(self, actionSpace: List[int], usesMouseClick: bool, gridSize=64, colorCount=16):
    ...
    self.backbone = nn.Sequential(
      nn.Conv2d(colorCount, 16, kernel_size=3, padding=1),
    ...
    if self.usesMouseClick:
      self.mouseClickConvolutions = nn.Sequential(
  ...
  def forward(self, x: torch.Tensor) -> torch.Tensor:
    ...
    actionLogits = self.actionHead(extractedFeatures)
...
class StochasticGoose():
  ...
  def __init__(self, available_actions, seed: int = 42):
    self.actionMap = {action.value: action for action in available_actions if action is not GameAction.RESET}
    ...
    # Seed model initialization and PyTorch action sampling.
    torch.manual_seed(seed)

    # Keep replay sampling independent of Python's global random state.
    random.seed(seed)

    self.usesMouseClick = any(action.is_complex() for action in self.actionMap.values())
  ...
  def choose_action(self, frame: np.ndarray, available_actions, episode_finished: bool = False, ):
    ...
    if self.previousFrame is not None and self.previousAction is not None:
      experience = {"state": self.previousFrame.cpu().numpy().astype(bool), "action": self.previousAction, "reward": (1.0 if not torch.equal(self.previousFrame, frameTensor) else 0.0), }
    ...
    if current_actions != set(self.actionSpace):
      raise RuntimeError("Available actions changed; the reference Goose "
